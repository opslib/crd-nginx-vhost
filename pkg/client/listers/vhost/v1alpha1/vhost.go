...
type VhostLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Vhost, err error)
...
// List lists all Vhosts in the indexer.
func (s *vhostLister) List(selector labels.Selector) (ret []*v1alpha1.Vhost, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Vhost))
	})
	return ret, err
}
...
type VhostNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.Vhost, error)
...
// List lists all Vhosts in the indexer for a given namespace.
func (s vhostNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Vhost, err error) {
...
// Get retrieves the Vhost from the indexer for a given namespace and name.
func (s vhostNamespaceLister) Get(name string) (*v1alpha1.Vhost, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("vhost"), name)
	}
	return obj.(*v1alpha1.Vhost), nil
}
